...
function BlogPostPage({ params }) {
    ...
    const relatedPosts = post.relatedPosts
        ? posts.filter((p) => { var _a; return (_a = post.relatedPosts) === null || _a === void 0 ? void 0 : _a.includes(p.id); })
        : [];
    return (<article className="pb-16">
      {/* Hero Section */}
      <div className="relative h-[40vh] overflow-hidden">
        <image_1.default src={`${post.featuredImage}?w=1920&h=600&fit=crop`} alt={post.title} fill className="object-cover" priority/>
      ...
      <div className="container max-w-4xl -mt-16 relative">
        ...
        <div className="flex justify-between items-center mb-4">
          <button_1.Button asChild variant="ghost" size="sm">
            <link_1.default href="/blog" className="flex items-center">
              <lucide_react_1.ChevronLeft className="h-4 w-4 mr-1"/>
              Back to Blog
            </link_1.default>
          </button_1.Button>
          <div className="flex gap-2">
            {(_a = post.categories) === null || _a === void 0 ? void 0 : _a.map((category) => (<link_1.default key={category} href={`/blog?category=${category.toLowerCase()}`} className="text-xs py-1 px-2 bg-muted rounded-full hover:bg-muted/80">
                {category}
              </link_1.default>))}
          </div>
        </div>
        ...
        {/* Post card */}
        <card_1.Card className="p-8 shadow-lg">
          <card_1.CardContent className="p-0">
            ...
            <div className="flex items-center justify-between flex-wrap mb-8 pb-6 border-b">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  {post.author.bio && (<div className="text-sm text-muted-foreground">
                      {post.author.bio}
                    </div>)}
                </div>
              </div>
            ...
            year: "numeric",
                ...
                {post.readingTime && (<div className="flex items-center text-sm text-muted-foreground">
            ...
            {/* Post content */}
            <div className="prose prose-zinc dark:prose-invert max-w-none mb-8" dangerouslySetInnerHTML={{
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-t pt-6 mt-8">
              ...
              <div className="flex gap-2">
                <button_1.Button size="icon" variant="outline" aria-label="Share on Facebook">
                  <lucide_react_1.Facebook className="h-4 w-4"/>
                </button_1.Button>
                <button_1.Button size="icon" variant="outline" aria-label="Share on Twitter">
                  <lucide_react_1.Twitter className="h-4 w-4"/>
                </button_1.Button>
                <button_1.Button size="icon" variant="outline" aria-label="Share on LinkedIn">
                  <lucide_react_1.Linkedin className="h-4 w-4"/>
                </button_1.Button>
                <button_1.Button size="icon" variant="outline" aria-label="Copy link">
        ...
        {relatedPosts.length > 0 && (<div className="mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {relatedPosts.map((relatedPost) => (<link_1.default key={relatedPost.id} href={`/blog/${relatedPost.slug}`} className="flex gap-4 items-start group">
                  <div className="relative h-16 w-24 rounded-md overflow-hidden flex-shrink-0">
                    <image_1.default src={`${relatedPost.featuredImage}?w=200&h=120&fit=crop`} alt={relatedPost.title} fill className="object-cover transition-transform group-hover:scale-105"/>
                  </div>
        ...
        {/* Comments section */}
        {post.comments && post.comments.length > 0 && (<div className="mt-12">
            ...
            <div className="space-y-6">
              {post.comments.map((comment) => (<div key={comment.id} className="flex gap-4 border-b pb-6">
                  <div className="relative h-10 w-10 rounded-full overflow-hidden flex-shrink-0">
                    <image_1.default src={comment.author.image} alt={comment.author.name} fill className="object-cover"/>
                  </div>
                  <div className="flex-1">
              ...
              <form className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" id="name" className="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm" required/>
                  </div>
                  <div>
                    <label htmlFor="email" className="block text-sm font-medium mb-1">
                      Email
                    </label>
                    <input type="email" id="email" className="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm" required/>
                  </div>
                </div>
                <div>
                  ...
                  <textarea id="comment" className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm min-h-[120px]" required/>
                </div>
                <button_1.Button type="submit">Submit Comment</button_1.Button>
              </form>
            </div>
          </div>)}
      </div>
